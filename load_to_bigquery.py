...
- inventory & sales = load into *_stg (WRITE_TRUNCATE), then MERGE into partitioned tables
...
CREDS_PATH = must_getenv("GOOGLE_APPLICATION_CREDENTIALS")
...
def load_data_to_table(
    ...
    job_config = bigquery.LoadJobConfig(
        ...
        # autodetect=False is fine since table schema exists already
        autodetect=False,
    )
...
def truncate_table(table_name: str) -> None:
    """TRUNCATE a table (cheap) so staging doesn't keep old rows on days with no data."""
    run_sql(f"TRUNCATE TABLE `{DATASET_ID}.{table_name}`")


def main():
    print("=" * 60)
    print("LOADING DATA TO BIGQUERY (with locations + staging + merge)")
    print("=" * 60)

    with open("sync_data.json", "r", encoding="utf-8") as f:
        data = json.load(f)

    # --- Full refresh dimension-like tables ---
    ...
    # Locations table (as before)
    # If your current sync_data.json doesn’t include locations, this will just skip.
    load_data_to_table("locations", data.get("locations", []), write_disposition="WRITE_TRUNCATE")

    # --- Facts via staging then MERGE (dedupe by IDs) ---
    ...
    print("\n🔁 Merging staging → partitioned tables (dedupe by primary IDs)...")

    # Inventory merge (partition-pruned by snapshot_date range in staging)
    if inventory_rows:
        run_sql(f"""
        ...
        MERGE `{DATASET_ID}.inventory_snapshots` T
        ...
        WHEN NOT MATCHED THEN
          INSERT (snapshot_id, variant_id, sku, location_id, available_qty, incoming_qty, committed_qty, snapshot_date, snapshot_timestamp)
          VALUES (S.snapshot_id, S.variant_id, S.sku, S.location_id, S.available_qty, S.incoming_qty, S.committed_qty, S.snapshot_date, S.snapshot_timestamp);
        """)

    # Sales merge (partition-pruned by sale_date range in staging)
    if sales_rows:
        run_sql(f"""
        ...
        MERGE `{DATASET_ID}.sales_history` T
        ...
        WHEN MATCHED THEN UPDATE SET
          ...
          vendor = S.vendor
        WHEN NOT MATCHED THEN
          INSERT (sale_id, order_id, order_name, variant_id, sku, product_title, quantity_sold, sale_date, sale_timestamp, vendor)
          VALUES (S.sale_id, S.order_id, S.order_name, S.variant_id, S.sku, S.product_title, S.quantity_sold, S.sale_date, S.sale_timestamp, S.vendor);
        """)

    print("\n✅ Load + merge complete!")
    print("Next: Verify in BigQuery (counts, latest snapshot_date, etc.)")
